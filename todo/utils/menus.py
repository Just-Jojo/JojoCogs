...
class ViewTodo(menus.Menu):
    ...
    def __init__(self, index: int, cache: Cache, todo_data: dict, **settings):
        self.index = index
        self.cache = cache
        self.data = todo_data
        self.settings = settings
    ...
    async def _format_page(self) -> Dict[str, Union[str, discord.Embed]]:
        ...
        if await self.ctx.embed_requested() and self.settings.get("use_embeds"):
            embed = discord.Embed(
                ...
                description=self.data["task"],
                timestamp=datetime.utcnow(),
            )
            embed.add_field(name="Pinned", value=self.data["pinned"])
            return {"embed": embed}
        ret = f"**{title}**\n\n{self.data['task']}\n\nPinned: {self.data['pinned']}"
        if self.settings.get("use_timestamps"):
            ret += f"\n{timestamp_format()}"
        return {"content": ret}

    @menus.button("\N{PUSHPIN}")
