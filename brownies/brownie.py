...
class Brownie(commands.Cog):
    ...
    async def nom(self, ctx):
        """Eat a brownie"""
        author = ctx.author
        user_brownies = await self.config.guild(ctx.guild).Players.author.get_raw("brownies")
        if user_brownies == 0:
    ...
    async def brownies(self, ctx):
        ...
        author = ctx.author
        brownies = await self.config.guild(ctx.guild).Players.author.get_raw("brownies")
    ...
    async def check_cooldowns(self, ctx: commands.Context, user, action) -> bool:
        ...
        if abs(await self.config.guild(user.guild).Players.get_raw(user, action) - int(time.perf_counter())) >= path:
