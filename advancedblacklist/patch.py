...
def init(bot: Red):
    ...
    async def remove_from_blacklist(
        self: WhitelistBlacklistManager, guild: discord.Guild, user_or_role: Set[int], *, dispatch: bool = True
    ):
        await _original_funcs["remove_from_blacklist"](self, guild, user_or_role)
        if dispatch:
            bot.dispatch("blacklist_remove", guild, user_or_role)

    async def clear_blacklist(self: WhitelistBlacklistManager, guild: discord.Guild, *, dispatch: bool = True):
    ...
    async def clear_whitelist(self: WhitelistBlacklistManager, guild: discord.Guild, *, dispatch: bool = True):
